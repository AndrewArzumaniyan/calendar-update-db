...
def get_events(customer_id: int):
  ...
  cur = conn.cursor()
  try:
    query = 'SELECT * FROM event WHERE customer_id = %s AND event_end > %s'
...
def update_event(event_id: str, event_name: str, event_start, event_end, event_last_modified):
  ...
  try:
    query = "UPDATE event SET event_name = %s, event_start = %s, event_end = %s, event_last_modified = %s WHERE event_id = %s"
